	...
	public class ConstraintCollection : AttachableCollection<Constraint> {
		...
		public override Task LoadAsync()
		{
			return ExecuteQueryAsync(
				...
				from sys.tables t
					inner join sys.indexes i on i.object_id = t.object_id and i.type_desc <> 'HEAP'
				...
				order by
					s.name, t.name, i.name, ic.key_ordinal, ic.index_column_id",
				dr => {
					string indexName = (string) dr["indexName"];

					var c = this.FirstOrDefault(i => i.Name == indexName);
					if (c == null) {
						c = new Constraint(indexName, "INDEX");
						Add(c);
					}
